...
Events are dicts (shared.interfaces.Event is a TypedDict), so fields are accessed
with ``event["field"]``.
"""
...
def test_event_has_exactly_the_contract_keys():
    event = parse_line(1, SAMPLE)
    assert set(event.keys()) == {
        "raw_text", "line_number", "service_name",
        "raw_timestamp", "severity", "message",
    }
